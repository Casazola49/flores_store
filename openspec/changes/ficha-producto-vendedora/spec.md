# Spec — Ficha de Producto Vendedora (`ficha-producto-vendedora`)

## State

- **Change:** `ficha-producto-vendedora` · **Branch:** `sdd/ficha-producto-vendedora`.
- **Form:** legacy flat (`openspec/changes/{change}/spec.md`) per repo convention; canonical form (`specs/{domain}/spec.md`) applies after archive.
- **Sources of truth:** `DESIGN.md` (One-Voice crimson, Sharp Rule, No-Yellow, focus-visible, reduced-motion), `PRODUCT.md` (ES-BO voice, WCAG 2.2 AA, transparent pricing, ethical persuasion), `convex/schema.ts` (live data contract), `src/lib/whatsapp.ts` (existing helpers), `openspec/config.yaml` (this change's IN/OUT boundaries).
- **Capability areas (assumed):** the proposal omits an explicit `Capabilities` section; the capability areas below are inferred from the affected surfaces and approved scope (PDP, truthful sales signals, favorites, category navigation, canonical card, seed batch, admin configuration). Treating this as a risk until product owner confirms.

## Capability area A — PDP Gallery
...
## Capability area B — PDP Size Guide Modal
...
## Capability area C — PDP Related Products
...
## Capability area D — Single-Product WhatsApp CTA
...
## Capability area E — Truthful Sales Signals
...
## Capability area G — Category Navigation
...
## Capability area H — Canonical Product Card
...
## Capability area I — Seed Data (+10)
...
## Capability area J — Admin Configuration
...
## Capability area K — Honest Signals (Cross-Cutting Normative Constraints)
