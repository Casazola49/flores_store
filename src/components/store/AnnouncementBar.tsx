...
const blocked = new Set(["#e5c400", "#ffd700", "#ffb300", "#ffc107"]);
export default function AnnouncementBar() {
  ...
  useEffect(() => { const date = (announcement as any)?.countdown_end_date; if (!date) return; const end = new Date(date).getTime(); if (end <= Date.now()) return; const tick = () => setRemaining(Math.max(0, end - Date.now())); tick(); const id = setInterval(tick, 1000); return () => clearInterval(id); }, [announcement]);
