...
export default function HomeClient() {
  ...
  return <div className="min-h-screen bg-[var(--color-primary)] text-white">
    ...
    <section className="bg-[var(--color-surface)] text-[var(--color-text)] px-6 py-20"><div className="max-w-[1400px] mx-auto"><div className="flex items-end justify-between mb-10"><div><p className="text-label text-[var(--color-accent)] mb-4">Explora</p><h2 className="text-headline font-bold">Compra por colección</h2></div><Link href="/productos" className="text-label text-[var(--color-accent)]">Ver todo →</Link></div><div className="grid sm:grid-cols-3 gap-6">{categories.slice(0, 3).map((cat: Category) => <Link key={cat.id} href={`/productos?category=${cat.slug}`} className="relative aspect-portrait overflow-hidden bg-[var(--color-primary)]">{cat.image_url ? <img src={cat.image_url} alt={cat.name} className="h-full w-full object-cover opacity-75" /> : <BrandPlaceholder aspect="4:5" label={cat.name} />}<span className="absolute bottom-5 left-5 text-title font-bold text-white">{cat.name}</span></Link>)}</div></div></section>
    <section className="max-w-[1100px] mx-auto px-6 py-20"><h2 className="text-headline font-bold mb-10">Compra con confianza</h2><div className="grid md:grid-cols-3 gap-8">{[[Truck, "Envíos 48h", "A todo Bolivia"], [ShieldCheck, "Pago seguro", "QR, transferencia y efectivo"], [RotateCcw, "Cambios 30 días", "Con stock real"]].map(([Icon, title, text]) => <div key={title as string}><Icon size={24} className="text-[var(--color-accent)] mb-5" /><h3 className="text-title font-bold mb-2">{title as string}</h3><p className="text-body text-[var(--color-text-muted)]">{text as string}</p></div>)}</div></section>
    <section className="bg-[var(--color-surface)] text-[var(--color-text)] px-6 py-20"><div className="max-w-[700px] mx-auto text-center"><h2 className="text-headline font-bold mb-4">Recibe novedades</h2><p className="text-body mb-8">Conoce nuevos modelos y disponibilidad de stock.</p><div className="flex gap-3"><input type="email" placeholder="Tu correo electrónico" className="min-w-0 flex-1 border-b border-[var(--color-text)] bg-transparent px-2 py-3 text-body" /><button className="btn-premium">Suscribirme</button></div></div></section>
  </div>;
}
