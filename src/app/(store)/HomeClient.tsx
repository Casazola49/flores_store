...
import ProductCard, { ProductCardData, mapProductToCardData } from "@/components/store/ProductCard";

const MARQUEE_ITEMS = ["Novedades", "Botas", "Tacos", "Zapatillas", "Envíos a Bolivia", "Pago con QR"];
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function toHot(p: any): ProductCardData {
  return mapProductToCardData(p);
}
...
export default function HomeClient() {
  ...
  return <div className="min-h-screen bg-[var(--color-primary)] text-white">
    ...
    <section className="max-w-[1400px] mx-auto px-6 py-20"><p className="text-label text-[var(--color-accent)] mb-4">Novedades</p><h2 className="text-headline font-bold mb-10">Selección Flores</h2><div className="grid grid-cols-2 lg:grid-cols-4 gap-6">{products.map(p => <ProductCard key={p.id} product={p} variant="dark" />)}</div></section>
